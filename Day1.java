...
//                "treb7uchet";
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            boolean haasSecondDig=false;
            for (int j = 0; j < arr[i].length(); j++) {
                char current=arr[i].charAt(j);

                if(Character.isDigit(current)){
                         int curIntValue=Integer.parseInt(String.valueOf(current));
                    if(!hasOneDigit) {
                        firstDigit = curIntValue;
                        hasOneDigit = true;
                    }else {
                        haasSecondDig=true;
                        secondDigit=curIntValue;
                    }
                }
            }
            if(!haasSecondDig){
            ...
            int firstIntVal=Integer.parseInt(String.valueOf(firstDigit));
            int secondIntVal=Integer.parseInt(String.valueOf(secondDigit));

            temp=firstIntVal*10+secondIntVal;

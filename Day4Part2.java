...
String input=   "Card   1: 79  1  6  9 88 95 84 69 83 97 | 42 95  1  6 71 69 61 99 84 12 32 96  9 82 88 97 53 24 28 65 83 38  8 68 79\n" +
...
//                        "Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11";
    ...
    List<String> winingParts = new ArrayList<>();
    List<String> scratchedParts = new ArrayList<>();
        for (int i = 0; i < arrayInput.length; i++) {
        String[] cardDevision = arrayInput[i].split(":");
        String[] devidedValues = cardDevision[1].trim().split("\\|");

        winingParts.add(devidedValues[0].trim());
        scratchedParts.add(devidedValues[1].trim());
    }

    List<List<Integer>>intWinList=new ArrayList<>();
    List<List<Integer>>intScratchedList=new ArrayList<>();
        for (int i = 0; i < winingParts.size(); i++) {
        List<Integer>tempWins=convertStringToIntArray(winingParts.get(i));
        intWinList.add(tempWins);
        List<Integer>tempScratched=convertStringToIntArray(scratchedParts.get(i));
        intScratchedList.add(tempScratched);
    }
        System.out.println("winList "+intWinList);
        System.out.println(" scrachedList "+intScratchedList);


        int[]arrAllMathes=collectAllMathes(intWinList,intScratchedList);
        getScretchCardNums(arrAllMathes);

}

    private static void getScretchCardNums( int[] arrAllMathes) {

        int[] countCards = new int[arrAllMathes.length];
        long sumCards=arrAllMathes.length;
        Arrays.fill(countCards,1);
        for (int i = 0; i < arrAllMathes.length; i++) {
            for (int k = 0; k < countCards[i]; k++) {
            for (int j = 0; j < arrAllMathes[i]; j++) {
                countCards[i + j + 1]++;
                sumCards++;
                }
            }
            System.out.println(Arrays.toString(countCards));
    ...
    public static List<Integer> convertStringToIntArray(String str){
        ...
        for (int i = 0; i < nums.length; i++) {
            String temp=String.valueOf(nums[i]).trim();
            if(!temp.isBlank()) {
    ...
    public static int[] collectAllMathes(List<List<Integer>>intWinList, List<List<Integer>>intScratchedList){

        int[]allMaches=new int[intScratchedList.size()];
        for (int i = 0; i <intScratchedList.size(); i++) {
            int matches=0;
            for (int j = 0; j <intScratchedList.get(i).size() ; j++) {
                if(intWinList.get(i).contains(intScratchedList.get(i).get(j))){
                    matches++;
                }
            }
            allMaches[i]=matches;
        ...
        System.out.println(Arrays.toString(allMaches));
        return allMaches;
    }
}
